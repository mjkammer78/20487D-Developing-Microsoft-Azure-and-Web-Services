...
6.  To run the project, run the following command:
    ```bash
        dotnet run
    ```
	NOTE: may need to trust development certificate, see https://www.hanselman.com/blog/DevelopingLocallyWithASPNETCoreUnderHTTPSSSLAndSelfSignedCerts.aspx
...
8. On the blade, select **Applications** and click **Add**.
 Note: Applications is now Applications (Legacy)
 ...
 Note1: during creation - choose => Supported account types - choose 'Accounts in any organisational directory or any identity provider. CHOOSE THIS TO AUTHENTICATE WITH AZURE AD B2C'
 Note2: under Authentication => Implicit grant, choose 'Access tokens' (and, optionally, 'ID tokens')
