...
namespace BlueYonder.Hotels.Service.Controllers
{
    ...
    {
        private HotelBookingRepository _repo;

        public HotelBookingController()
        {
            _repo = new HotelBookingRepository();
        }
        ...
        {
            Booking booking = await _repo.GetBooking(id);
            if (booking != null)
                return Ok(booking);
            else
                return NotFound();
        ...
        public async Task<ActionResult> Post([FromBody] Booking booking)
        {
            ...
            Booking BookingDb = await _repo.Add(booking);
            if (BookingDb != null)
                return Ok(BookingDb);
            else
        ...
        public async Task<ActionResult<Booking>> Put(int id, [FromBody] Booking booking)
        {
            ...
            Booking updatedBooking = await _repo.Update(booking);
            return Ok(updatedBooking);
        }

        
        // DELETE api/HotelBooking/1
        [HttpDelete("{id}")]
